...
def login():
    ...
    for row in rows:
        if request.form['email'] == row[2] and request.form['pwd'] == row[3]:
            ...
            cur2.execute("SELECT * FROM dispositivos WHERE idUsuario = " + str(row[0]))
            rows2 = cur2.fetchall()
...
def cadastrarLoginDB():
    cur.execute("INSERT INTO login VALUES(%s, %s, %s, %s)", (3, request.form['nome'], request.form['email'], request.form['pwd']))
    conn.commit()
